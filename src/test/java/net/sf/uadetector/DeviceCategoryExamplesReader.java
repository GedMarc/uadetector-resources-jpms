...
final class DeviceCategoryExamplesReader
{
	 ...
	 * Default log
	 */
	private static final java.util.logging.Logger LOG = java.util.logging.Logger.getLogger(DeviceCategoryExamplesReader.class.toString());
	...
	public static List<DeviceCategoryExample> read()
	{
		...
		List<DeviceCategoryExample> examples = new ArrayList<>(2000);
		if (csvParser != null)
		{
			String[] line = null;
			int i = 0;
			do
			{
				...
				if (line != null)
				{
					...
					{
						LOG.warning("Can not read device category example " + i + ", there are too few fields: " + line.length);
					}
				}
			}
			while (line != null);
