...
public class UserAgentStringParserIntegrationTest
{
	 ...
	 * Default log
	 */
	private static final java.util.logging.Logger LOG = java.util.logging.Logger.getLogger(UserAgentStringParserIntegrationTest.class.toString());
	...
	{
		int i = 0;
		for (DeviceCategoryExample example : DC_EXAMPLES)
		{
			ReadableUserAgent agent = PARSER.parse(example.getUserAgentString());

			// abort if category is unknown
			String msg1 = "Unknown device category for: " + example.getUserAgentString();
			assertThat(agent.getDeviceCategory()
			                .getCategory()).as(msg1)
			                               .isNotEqualTo(Category.UNKNOWN);

			// abort if category is unknown
			String msg2 = "Different device category for: " + example.getUserAgentString();
			assertThat(agent.getDeviceCategory()
			                .getName()).as(msg2)
			                           .isEqualTo(example.getCategory());
